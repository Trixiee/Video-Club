...
%>
	...
	<div class="container-fluid">
		...
		<div class="container">
		<form action="/LibraryWeb/PrikazZaduzenjaSerijaServlet"
			method="get">
			<div class="form-group">
				<label for="idClana">Unesite id clana:</label> <input type="text"
					class="form-control" id="idClana" placeholder="idClana"
					name="idClana">
			</div>
			<button type="submit" class="btn btn-default">Prikazi</button>
		</form>
	<c:if test="${!empty zaduzenjaserija}">
		<table class="table table-bordered">
			...
			<tbody>
			<c:forEach var="z" items="${zaduzenjaserija}">
				<tr>
					<td>${z.idZaduzenjaSerije}</td>
					<td>${z.datumZaduzenja}</td>
					<td>${z.datumVracanjaSerije}</td>
					<td>${z.clan.idClan}</td>
					<td>${z.primerakserija.invBrojSerije}</td>
				</tr>
			</c:forEach>
			</tbody>
		</table>

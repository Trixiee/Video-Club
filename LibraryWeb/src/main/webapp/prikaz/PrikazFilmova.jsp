...
%>
	<jsp:useBean id="filmoviBean" class="beans.GetSveFilmoveBean"
	...
	<div class="container">
		<table class="table">
			...
			<c:forEach var="f" items="${filmoviBean.sviFilmovi}">
				<tr>
					<td>${f.idFilma}</td>
					<td>${f.naslov}</td>
					<td>${f.autor}</td>
					<td>${f.datumProizvodnjeFilma}</td>
					<td>${f.produkcijskaKuca}</td>
					<td>${f.zanr.nazivZanra}</td>
				</tr>
			</c:forEach>
		</table>
		
		<form action="/LibraryWeb/ReportServletFilm">

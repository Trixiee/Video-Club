...
%>
	<jsp:useBean id="serijeBean" class="beans.GetSveSerijeBean"
	...
	<div class="container">
		<table class="table">
			<tr>
				...
				<th>Naslov</th>
				<th>Autor</th>
				<th>Datum proizvodnje filma</th>
				<th>Produkcijska kuca</th>
				<th>Broj epizoda</th>
				<th>Zanr</th>

			</tr>
			<c:forEach var="s" items="${serijeBean.sveSerije}">
				<tr>
					<td>${s.idSerija}</td>
					<td>${s.naslov}</td>
					<td>${s.autor}</td>
					<td>${s.godinaProizvodnjeSerije}</td>
					<td>${s.produkcijsaKuca}</td>
					<td>${s.brojEpizodaSerije}</td>
					<td>${s.zanr.nazivZanra}</td>
				</tr>

			</c:forEach>
		</table>
		
		<form action="/LibraryWeb/ReportServletSerija">
			<button type="submit" class="btn btn-default">Generisi izevstaj</button>
		</form>
	</div>
</body>
</html>
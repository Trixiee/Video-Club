...
%>
	...
	<div class="container">
		<form action="/LibraryWeb/PrikaziZaduzenjaFilmovaZaClanaServlet"
			method="get">
			<div class="form-group">
				<label for="idClana">Unesite id clana:</label> <input type="idClana"
					class="form-control" id="idClana" placeholder="idClana"
					name="idClana">
			</div>
			<button type="submit" class="btn btn-default">Prikazi</button>
		</form>
	<c:if test="${!empty zaduzenjafilma}">
		<table class="table table-bordered">
			...
			<tbody>
			<c:forEach var="z" items="${zaduzenjafilma}">
				<tr>
					<td>${z.idZaduzenjaFilma}</td>
					<td>${z.datumZaduzenjaFIlma}</td>
					<td>${z.datumVracanjaFilma}</td>
					<td>${z.clan.idClan}</td>
					<td>${z.primerakfilm.invBrojFilma}</td>
				</tr>
			</c:forEach>
			</tbody>
		</table>
